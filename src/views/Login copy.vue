<template>
  <div class="login">
    <van-nav-bar left-text="返回" left-arrow @click-left="goState('Home')" />
    <div class="nav-left">
      <div class="logo"></div>
      <div class="logo-text">leisure time</div>
    </div>
      ...
      <van-form>
        ...
        <div class="forgot clearfix">
          <span class="ipt-sp fl">
            <input type="checkbox" v-model="isCheck" @click="checkbox" />
            记住密码
          </span>
...
export default {
  ...
  created() {
      this.$nextTick(() => {
            ...
            let phoneIpt = this.$refs.phone.$refs.input.value;
            let passwordIpt = this.$refs.password.$refs.input.value;
            let user = localStorage.getItem("__key", this.userInfo.phone);
            let password = localStorage.getItem("__password", this.userInfo.password);
            // 判断输入框是否有值
            if (user != '', password != '') {
                // 设置输入框的值
                phoneIpt = user;
  ...
  methods: {
    ...
    checkbox() {
      ...
      if (this.userInfo.phone != "" && this.userInfo.password != "") {
         this.phoneIpt = this.userInfo.phone;
         this.passwordIpt = this.userInfo.password;
        if (this.isCheck) {
          //  记住账号
          localStorage.setItem("__key", this.userInfo.phone);
          //  记住密码
          localStorage.setItem("__password", this.userInfo.password);
        } else {
          // 不用记住账号,密码
